...
KAGGLE_DATASET = "nikhil7280/student-performance-multiple-linear-regression"
CSV_FILENAME = "Student_Performance.csv"  # CSV file name inside dataset
LOCAL_DIR = os.path.join(os.getcwd(), "student_performance_dataset")
PARQUET_FILE = os.path.join(LOCAL_DIR, "Student_Performance.parquet")
...
def clean_dataframe(df):
    ...
    # Convert yes/no to 1/0 if column exists (BEFORE lowercase conversion)
    if "Extracurricular Activities" in df.columns:
        df["Extracurricular Activities"] = df["Extracurricular Activities"].map({"Yes": 1, "No": 0})

    # Strip whitespace in all string/object cells
